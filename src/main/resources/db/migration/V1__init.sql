...
(
    expenses_id UUID PRIMARY KEY not null,
    category_id UUID REFERENCES expense_categories(category_id) not null,
    description varchar(500),
    cost real not null,
...
CREATE TABLE users_income_categories(
                                        user_id UUID REFERENCES users(user_id) ON DELETE CASCADE not null,
                                        category_id UUID REFERENCES income_categories(category_id ) ON DELETE CASCADE
);

CREATE TABLE single_incomes
(
    income_id UUID PRIMARY KEY not null,
    category_id UUID REFERENCES income_categories(category_id) not null,
    description varchar(500),
